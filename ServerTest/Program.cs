        ...
        private static void sendEmail(Person p)
        {
            try
            {
                ...
                using (SmtpClient client = new SmtpClient("smtp.gmail.com", 465))
                {
                    ...
                    client.Credentials = new NetworkCredential("something@gmail.com", "");

                    client.Send(
                        new MailMessage("something@abv.bg", p.Email,
                            "Thank you for using the Web Service",
                            string.Format(@"Thank you for using our Web Service, {0}. We have recieved your message, '{1}'.", p.Name, p.Message) )
                    );
                }
